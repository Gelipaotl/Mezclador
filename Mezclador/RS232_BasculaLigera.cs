...
namespace Mezclador
{
    public static class RS232_BasculaLigera
    {
        ...
        public static void Connect()
        {
            ...
            portName = UserSettings.COM_BasculaLigera;
        ...
        private static void SerialPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            if (serialPort != null && serialPort.IsOpen)
            {
                ...
                try
                {
                    ...
                    //{
                    //if (receivedString.EndsWith("kg"))
                    //{
                    //    receivedData = weight = receivedString.Substring(4).Trim();
                    //}
                    //}
                    //if (receivedString.Length > 10 && receivedString.EndsWith("kg"))
                    //{
                    //receivedData = receivedString;

                    //var firstChar = receivedString[0];
                    //                  var units = receivedString.Substring(9, 2);
                    //string weightString = receivedString.Substring(3, 5).Trim();

                    //if (firstChar == ' ' && units == "kg")
                    //{
                    //if (receivedString.EndsWith("\r\n"))
                    //{
                    if ((receivedString.IndexOf('k') - 7) > 0)
                    {
                        var index = receivedString.IndexOf('k') - 7;
                        if (receivedString[index + 2] == '.')
                            receivedData = weight = receivedString.Substring(index, 6).Trim();
                    }
                    //receivedData = 
                    //weight = receivedString.Substring(2, 6).Trim();
                    //}
